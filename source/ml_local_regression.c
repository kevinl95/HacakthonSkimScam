...
#include MTB_ML_INCLUDE_MODEL_X_DATA_FILE(RSSIMODEL)
#include MTB_ML_INCLUDE_MODEL_Y_DATA_FILE(RSSIMODEL)
...
/*******************************************************************************
* Function Name: ml_local_regression_input
********************************************************************************
* Summary:
*   Run the Neural Network Inference Engine for a given input.
*
* Return:
*   The inference.
*******************************************************************************/
int32_t ml_local_regression_input(uint32_t val)
{
    /* Regression pointers */
    ...
    input_reference  = (MTB_ML_DATA_T *) val;
    output_reference = (int32_t *) MTB_ML_MODEL_Y_DATA_BIN(RSSIMODEL);
    Cy_ML_Model_Inference(model_obj->inference_obj, input_reference, result_buffer, NULL);
    return output_reference[0];
}
...
cy_rslt_t ml_local_regression_task(void)
{
    ...
    /* Point to regression data */
    input_reference  = (MTB_ML_DATA_T *) MTB_ML_MODEL_X_DATA_BIN(RSSIMODEL);
    output_reference = (int32_t *) MTB_ML_MODEL_Y_DATA_BIN(RSSIMODEL);
